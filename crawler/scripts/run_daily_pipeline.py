...
from crawler.models.schemas import ChangeEvent, ProductRecord, UrlTask
...
def run_pipeline(html_pages: list[tuple[str, str]], db_path: str = "data/crawler.db") -> tuple[list[ChangeEvent], CrawlMetrics]:
...
def run_from_urls(urls: list[str], db_path: str = "data/crawler.db", raw_dir: str = "data/raw") -> tuple[list[ChangeEvent], CrawlMetrics]:
    fetcher = HttpFetcher()
    ...
    events, metrics = run_pipeline(html_pages, db_path=db_path)
...
def run_end_to_end(seed_url: str, db_path: str = "data/crawler.db", raw_dir: str = "data/raw", max_urls: int = 50) -> dict:
    ...
    events, metrics = run_from_urls(urls=urls, db_path=db_path, raw_dir=raw_dir)
    return {
        "seed_url": seed_url,
...
def main() -> None:
    ...
    if args.offline:
        ...
        events, metrics = run_pipeline(sample, db_path=args.db_path)
